...
for ((i = 0; i < radar_num; i++)); do
    ...
    # ----- ZPRIME
        ...
        if [ "$runtime_mode_zprime" == "live-edge" ]; then # ----- ZPRIME EDGE
            ...
            if [ "$mode_id" == "1" ]; then
                echo "You selected mode 1."
                json_param="./zadar_config/zprime/w_85m_512_config.json"
                bin_param="./zadar_config/zprime/r_85m_512_config.bin"
            elif [ "$mode_id" == "2" ]; then
                echo "You selected mode 2."
                json_param="./zadar_config/zprime/w_250m_512_config.json"
                bin_param="./zadar_config/zprime/r_250m_512_config.bin"
            elif [ "$mode_id" == "3" ]; then
                echo "You selected mode 3."
                json_param="./zadar_config/zprime/w_400m_512_config.json"
                bin_param="./zadar_config/zprime/r_400m_512_config.bin"
            else
                echo "[ERROR] Invalid option for mode_id: $mode_id"
                exit 1
            fi
...
if [ "$connection_ethernet" == true ]; then
    ...
    for idx in "${!zprime_sns[@]}"; do
        python3 lib/zprime.py --sn "${zprime_sns[$idx]}" --run-mode "${zprime_modes[$idx]}" &
        parallel_pids+=("$!")
    done
fi
sleep 2
...
if [ "$record_always" == "true" ]; then
    ...
    command_record="lib/Zadar_Record_csv-$arch_type.AppImage --radar_number $radar_num --scan off --raw_scan off --odometry off --clusters off --tracks on &"
    echo "$command_record"
    eval "$command_record"
    record_process_pid=$!
fi
...
command="lib/Zadar_Run-$arch_type.AppImage --radar_num $radar_num --radar_sn $radar_sn --json $json_param --bin $bin_param --enable_viewer $enable_viewer --enable_log $enable_log --runtime_mode_zprime $runtime_mode_zprime $override_timestamp_cmd"
echo "$command"
{
